...
func NewService(baseURL string, db string, errorFile *os.File) *Service {
	return &Service{
		baseURL:   baseURL,
		errorFile: errorFile,
		db:        db,
...
func (s *Service) WriteToSyncGateway(users *[]CouchbaseSgwUser, results chan string, jobs chan int, workerId int) {
	client := &http.Client{}
	...
	for job := range jobs {
		...
		req.Header.Set("Content-Type", "application/marshalledJson")
		...
		if resp.StatusCode != 201 {
			...
			results <- fmt.Sprintf("Worker %d: finished migrating %s", workerId, user.Name)
		}
	}
}
